...
function LogIn() {
  ...
  return (
    <main className="h-screen flex flex-col justify-start items-center gap-20 dark:text-white">
      <h1 className="text-6xl py-10">Welcome Back Traveller</h1>
      <form action="" className="border flex flex-col h-96 w-96 ">
        ...
        <button onClick={handleSubmit} className="border border-purple-700">
          Log In{" "}
        </button>
        {err && (
          <p className="text-center font-bold text-red-500">{err.error}</p>
        )}
      </form>
      <span>
        No account yet?{" "}
        <Link to="/SignUp" className="text-blue-800 font-extrabold">
          Sign up!
        </Link>
      </span>
    </main>
